...
const chartAlertingChecker = (data: { timestamp: number; values: number[] }[], alerts: IAlerts[], dataset: any, selectedMeasures: any[] ) => {
  ...
  allAlerts = {...allAlerts, [alerts[x].name]: Array.from(acceptedValues.values())}
}
}
return allAlerts;
};
...
export const alertingPlotBandsCreator = (vals: {}) => {
  ...
  for(const [key, values] of Object.entries(vals)){
    vals[key].map((val, idx) => plotBands.push(
      {
            color: grey[400],
            from: val[0],
            to: val[1],
