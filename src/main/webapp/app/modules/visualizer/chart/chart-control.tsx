  ...
  return (
    <Grid container direction="row" sx={{ mb: 1 }}>
      <Grid item alignItems="center" sx={{ display: 'flex', flexGrow: 1, flexDirection: 'row' }}>
        ...
        <FormGroup sx={{ml: 1}}>
          <FormControlLabel control={<Checkbox onChange={() => {props.updateLiveData()}} checked={liveData} defaultChecked />} label="Enable live data (demo)" />
        </FormGroup>
      </Grid>
      <Grid item>
        <Button
          ...
          sx={{ mr: 1, color: changeChart ? '#424242' : '#0277bd' }}
        >
          Overlay
        </Button>
        <Button
      ...
      {showDatePick && (
        <ChartDatePicker
          ...
          setShowDatePick={props.setShowDatePick}
          from={from}
          to={to}
